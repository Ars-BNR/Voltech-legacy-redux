...
const MakingOrder = () => {
    ...
    const handleChange = useCallback((target) => {
        if (target.name === "number") {
            const phoneNumber = parsePhoneNumberFromString(target.value, 'RU');
            if (phoneNumber) {
    ...
    return (
        <div className={classes.makingOrder}>
            ...
            <form className={classes.Content__dataConsignee} action="" onSubmit={(e) => e.preventDefault()}>
                <TextField
                    type="text"
                    name="name"
                    value={order.name}
            ...
            <p className={classes.Content__quantityProduct}>
                {order.allCount}{" "}
                {RenderPhrase(order.allCount)}
            </p>
            <div className={classes.Content__resultOrder}>
                <p className={classes.Content__total}>Итого: </p>
                <p className={classes.Content__price}> {order.price.toLocaleString('ru-RU')} </p>
